...
class ChatGPTBot(Bot, OpenAIImage):
    ...
    def __init__(self):
        ...
        client_arg = {}
        client_arg["api_key"] = conf().get("open_ai_api_key")
        if conf().get("open_ai_api_base"):
            client_arg["base_url"] = conf().get("open_ai_api_base")
        # proxy = conf().get("proxy")
        # if proxy:
        #     openai.proxy = proxy
        print(client_arg)
        self.client = OpenAI(**client_arg)
        ...
        self.args = {
            ...
            "temperature": conf().get("temperature",
                                      0.9),  # 值在[0,1]之间，越大表示回复越具有不确定性
            # "max_tokens":4096,  # 回复最大的字符数
            "top_p": conf().get("top_p", 1),
            "frequency_penalty": conf().get("frequency_penalty",
                                            0.0),  # [-2,2]之间，该值越大则更倾向于产生不同的内容
    ...
    def reply(self, query, context=None):
        ...
        if context.type == ContextType.TEXT:
            ...
            logger.debug(
                "[CHATGPT] new_query={}, session_id={}, reply_cont={}, completion_tokens={}"
                .format(
                    session.messages,
                    session_id,
            ...
            if reply_content["completion_tokens"] == 0 and len(
                    reply_content["content"]) > 0:
                reply = Reply(ReplyType.ERROR, reply_content["content"])
            elif reply_content["completion_tokens"] > 0:
                self.sessions.session_reply(reply_content["content"],
                                            session_id,
    ...
    def reply_text(self,
        ...
        try:
            if conf().get(
                    "rate_limit_chatgpt") and not self.tb4chatgpt.get_token():
...
class AzureChatGPTBot(ChatGPTBot):
    ...
    def create_img(self, query, retry_count=0, api_key=None):
        ...
        url = "{}dalle/text-to-image?api-version={}".format(
            openai.api_base, api_version)
        api_key = api_key or openai.api_key
        headers = {"api-key": api_key, "Content-Type": "application/json"}
        try:
            body = {
                "caption": query,
                "resolution": conf().get("image_create_size", "256x256")
            }
            ...
            while status != "Succeeded":
                logger.info("waiting for image create..., " + status +
